...
def train_model(network_data, feature_dic, log_name):
    ...
    with graph.as_default():
        ...
        if feature_dic is not None:
            ...
            feature_weights = tf.Variable(tf.random.truncated_normal([feature_dim, embedding_size], stddev=1.0))

            embed_trans = tf.Variable(tf.truncated_normal([feature_dim, embedding_size], stddev=1.0 / math.sqrt(embedding_size)))
            u_embed_trans = tf.Variable(tf.random.truncated_normal([edge_type_count, feature_dim, embedding_u_size], stddev=1.0 / math.sqrt(embedding_size)))
        else:
        ...
        for epoch in range(epochs):
            ...
            for i in range(edge_type_count):
                if args.eval_type == 'all' or edge_types[i] in args.eval_type.split(','):
                    tmp_auc, tmp_pr, tmp_f1 = evaluate(final_model[edge_types[i]], valid_true_data_by_edge[edge_types[i]], valid_false_data_by_edge[edge_types[i]])
                    valid_aucs.append(tmp_auc)
                    valid_f1s.append(tmp_f1)
                    valid_prs.append(tmp_pr)

                    tmp_auc, tmp_pr, tmp_f1 = evaluate(final_model[edge_types[i]], testing_true_data_by_edge[edge_types[i]], testing_false_data_by_edge[edge_types[i]])
            ...
            if cur_score > best_score:
                ...
                test_score = (average_auc, average_pr, average_f1)
                patience = 0
            else:
                patience += 1
...
if __name__ == "__main__":
    ...
    training_data_by_type = load_training_data('train.txt')
    valid_true_data_by_edge, valid_false_data_by_edge = load_testing_data('valid.txt')
    testing_true_data_by_edge, testing_false_data_by_edge = load_testing_data('test.txt')

    average_auc, average_pr, average_f1 = train_model(training_data_by_type, feature_dic, log_name + '_' + time.strftime('%Y-%m-%d %H-%M-%S',time.localtime(time.time())))
    valid.append(average_auc)
    valid.append(average_pr)
    valid.append(average_f1)
    save_walks("exit0_1", valid)
